...
class GeradorCurriculo:
    ...
    def adicionar_experiencia(self):
        ...
        for exp in self.dados["experiencias"]:
            ...
            self.doc.add_paragraph(
                f"{exp['cargo']} | {exp['periodo']} | {exp['local']}"
            )
    ...
    def gerar_curriculo(
        self, nome_arquivo_docx="Curriculo_Raimundo_Marques.docx", sobrescrever=True
    ):
        try:
            pasta_destino = "Docs"
            ...
            if not sobrescrever and (
                os.path.exists(caminho_docx) or os.path.exists(caminho_pdf)
            ):
                resposta = input(
                    f"Arquivos já existem em {pasta_destino}. Deseja sobrescrever? (s/n): "
                ).lower()
            ...
            if sistema == "windows":
                if docx2pdf_convert is not None:
                    try:
                        ...
                        print(
                            f"DOCX gerado, mas falha ao converter para PDF (docx2pdf): {conv_err}"
                        )
                else:
                    print(
                        "docx2pdf não está instalado. Instale com 'pip install docx2pdf'."
                    )
            else:
                ...
                try:
                    ...
                    print(
                        f"DOCX gerado, mas falha ao converter para PDF (libreoffice): {conv_err}"
                    )
            ...
            if pdf_gerado and os.path.exists(caminho_pdf):
                ...
                try:
                    ...
                    if tamanho_depois <= 4:
                        ...
                        reducao = (
                            (tamanho_antes - tamanho_depois) / tamanho_antes
                        ) * 100
                        print(
                            f"✅ PDF comprimido: {tamanho_antes:.2f}MB → {tamanho_depois:.2f}MB (redução {reducao:.1f}%)"
                        )
                    else:
                        print(
                            f"⚠️ Mesmo após compressão, PDF tem {tamanho_depois:.2f}MB (> 4MB)"
                        )
                except Exception as e:
...
def main():
    dados_pessoais = {
        ...
        "endereco": "Travessa 3 nº 4, Flores - Manaus/AM - CEP: 69028-341",
        ...
        "formacao": [
            {
                "curso": "Pós-Graduação Lato Sensu/Especialização em Engenharia de Software",
